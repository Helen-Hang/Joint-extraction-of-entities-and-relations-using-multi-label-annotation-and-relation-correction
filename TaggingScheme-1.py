...
def Static(source_train_json, isTrain=True):
    ...
    for line in sentences:
        ...
        sentText = str(unicodedata.normalize('NFKD', sent['sentText'])
...
def tag_sent(source_json, tag_json, isTrain=True):
    ...
    """
    train_json_file = open(tag_json, 'w', -1)
    ...
    for line in sentences:
        ...
        sentText = str(sent['sentText']).rstrip('\n').rstrip('\r')
        try:
            tags = []
            tokens = nltk.word_tokenize(sentText)
            for i in range(0, len(tokens)):
...
if __name__ == "__main__":
    ...
    infile2 = "./data/demo/train_tag-1.json"
